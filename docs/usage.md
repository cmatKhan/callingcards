...
For the sake of reproducibilility and re-use, you can create a `params.yml` file which
can be passed with the `-params-file` flag. A params file in a yaml format uses the
cmd line parameters as keys and the values as the values. For example:
